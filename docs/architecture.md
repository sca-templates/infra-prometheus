...
| redis-exporter         | `oliver006/redis_exporter:v1.62.0`              | 127.0.0.1:9121 | kafka-network                | Redis metrics via `redis:6379`                                               |
...
- The **exporters** join the shared external Docker network **`kafka-network`**: postgres-exporter resolves `postgres-app-db:5432` by name, redis-exporter resolves `redis:6379` by name, and kafka-connect-exporter resolves `kafka-connect:8778` by name.
- **Redis** is on `kafka-network`; redis-exporter reaches it as `redis:6379` (`REDIS_ADDR=redis:6379`), no host gateway needed.
...
- The `.env` file is generated by `scripts/gen-env.sh` from Vault `secret/postgres-app/dev` + `secret/redis/dev` (read via the `prometheus` AppRole) and carries `DATA_SOURCE_NAME` (Postgres DSN rewritten to `postgres-app-db:5432`), `REDIS_ADDR` and `REDIS_PASSWORD`. It is gitignored, `chmod 600`.
