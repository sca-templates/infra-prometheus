...
- `.env` is gitignored and generated by `scripts/gen-env.sh` from Vault
  `secret/postgres-app/dev` + `secret/redis/dev` (`DATA_SOURCE_NAME`,
  `REDIS_PASSWORD`), `chmod 600`.
...
- Network: `prometheus` runs on the **host network** (`network_mode: host`) and
  ...
  **exporters** join the external `kafka-network` (name resolution);
  `redis` is on `kafka-network` → reached by redis-exporter as `redis:6379`.
- Consul registers services with TCP checks on `127.0.0.1:<port>` in
  `../consul/scripts/register-services.sh` and validates them in
  `../consul/scripts/validate.sh` (both read the single catalog
  `../consul/scripts/services.txt`).
- Microservice jobs (9101–9104) and the broker kafka-exporter job stay
  **commented** until a real target exists (`apache/kafka:3.7.1` ignores
  ...
  kafka stack.
- The `vault` and `kafka-connect` scrapes are **strict** in
  `scripts/validate.sh`; the vault scrape works because the `vault/` HCL config
  sets `telemetry { prometheus_retention_time = "24h" }`, and kafka-connect
  exposes JMX on `8778` (enabled in `kafka/docker-compose.yml`). Only `kong`
  (no stack yet) is tolerated down.
